...
```markdown
# 📊 UK Biobank Data Cleaning Pipeline Report: PDF-to-Markdown Post-Processing for SLATE
...
```
Pipeline: PDF → Marker → Markdown → Cleaner → Chunker → Tagger → Vectorizer → DuckDB → SLATE
                                      ↑
                                 [WE ARE HERE]
```
...
```python
PolicyMDCleaner:
├── fix_ligatures()         # Unicode ligature replacement (ﬁ→fi)
├── dehyphenate()           # Word-break repair across lines
├── normalize_whitespace()  # Space/newline standardization
├── standardize_headings()  # H4+ → H3 max normalization
├── remove_pdf_artifacts()  # OCR noise removal
├── fix_punctuation_spacing() # [CRITICAL: 759K issues remaining]
├── clean_and_collect_artifacts() # [BUG: Data loss from span tags]
└── count_metrics_after_char_start() # Post-processing metrics
```
...
```python
# Protected "4.0" → couldn't add space before "Our"
...
**Protection→Fix→Restore Cycle Failed**: Aggressive punctuation fixes couldn't operate on protected content
...
STEP 1: Protect "1.0E-8" → "__PROT_00001__However"
STEP 2: Fix ".H" → ". H" (but can't - it's protected)
STEP 3: Restore → "1.0E-8However" (no improvement)
```
...
```python
# NEW PIPELINE
text → spaCy NLP → Sentence Segmentation → Boundary Correction → 
       Token Analysis → Context-Aware Cleaning → Validated Output
```
...
├── 4_tagging/
│   ├── 1_chunk_enhanced/
│   │   └── 8_md_cleaner3/  [CURRENT]
│   └── 2_spacy_sentences/   [PROPOSED]
└── 5_vectorization/
    └── duckdb_storage/
```
...
```python
text = re.sub(r'\b\d+\.\d+(?=[\s,;:!?)\]}\-]|$)', protect_pattern, text)
text = re.sub(r'\b\d+\.?\d*\s*[-–—]\s*\d+\.?\d*\b', protect_pattern, text)
text = re.sub(r'\b\d+\.?\d*%', protect_pattern, text)
```

**Citation Cleanup (Lines 183-190)**:
```python
line = re.sub(r'\[\[?(\d+(?:[,\s–-]+\d+)*)\]?\]\(#page[^)]+\)', r'[\1]', line)
